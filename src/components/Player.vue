...
export default {
  ...
  created() {
    this.$ga.event(this.$route.params.town, "player", "created", 0)
  ...
  methods: {
    stop() {
      this.sound.stop()
      this.$ga.event(this.$route.params.town, "player", "stop", this.seek)
    ...
    pause() {
      this.sound.pause();
      this.isPlaying = false;
      this.$ga.event(this.$route.params.town, "player", "pause", this.seek)
    },
    polling() {
      ...
      if (this.seek === (this.info.length - 60)) {
        this.$ga.event(this.$route.params.town, "player", "finished", this.seek)
      }
    },
    play() {
      this.$ga.event(this.$route.params.town, "player", "play", this.seek)
      this.sound.play();
      this.isPlaying = true;
    }
  },
